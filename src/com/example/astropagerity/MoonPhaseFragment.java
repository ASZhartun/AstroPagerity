...
public class MoonPhaseFragment extends FragmentActivity {
	...
	private ArrayList<HashMap<String, String>> getParamCalculations(String dateForm) {
		...
		HashMap<String, String> jdMap = new MoonPhaseParameterItem("Юл. дата", Double.toString(jd));

		double[] pp = new double[6];
		Phase.phase(jd, pp);

		int phase = (int) (pp[0] * 100);
		HashMap<String, String> phaseMap = new MoonPhaseParameterItem("Фаза", Integer.toString(phase));

		String grow = (int) pp[1] + "д " + (int) (24 * (pp[1] - Math.floor(pp[1]))) + "ч "
				+ (int) (1440 * (pp[1] - Math.floor(pp[1]))) % 60 + "м";
		HashMap<String, String> growMap = new MoonPhaseParameterItem("Возраст", grow);
	...
	private ArrayList<HashMap<String, String>> getPhasesCalculations(String dateForm) {
		...
		HashMap<String, String> newMoonBegin = new MoonPhaseParameterItem("Новолуние: ", new Jdays(pp[0]).format(10));
		HashMap<String, String> firstQuarter = new MoonPhaseParameterItem("1 четверть: ", new Jdays(pp[1]).format(10));
		HashMap<String, String> fullMoon = new MoonPhaseParameterItem("Полнолуние: ", new Jdays(pp[2]).format(10));
		HashMap<String, String> thirdQuarter = new MoonPhaseParameterItem("3 четверть: ", new Jdays(pp[3]).format(10));
		HashMap<String, String> newMoonEnd = new MoonPhaseParameterItem("Новолуние: ", new Jdays(pp[4]).format(10));
